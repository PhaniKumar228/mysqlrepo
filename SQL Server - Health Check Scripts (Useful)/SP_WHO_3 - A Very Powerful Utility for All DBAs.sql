...
SP_WHO_3 is a very powerful utility for all DBAs. 
It displays processes connected to the system and what they are doing. 
It can find blocking processes, can return the Input Buffer for everything in the recordset returned, 
and provides immense filtering and sorting capabilities. 

It's designed to be extremely efficient, yet it provides many more features than sp_who, sp_who2, sp_lock, sp_lockinfo, 
and dbcc input buffer combined. Run sp_who_3 '?' for a complete description of how to use. 
This procedure requires SQL 2000 with SP3 to run. 
...
IF @run_mode IN (N'help', N'?')
	BEGIN
		PRINT ('
AUTHOR			: test name
AUTHOR EMAIL		: testname@testmail.com
...
-------------------------------------------------------------------------------------------------------
-- Use the following script to uninstall sp_who_3; just uncomment this section out to uninstall.
-------------------------------------------------------------------------------------------------------
-- USE master
-- SET QUOTED_IDENTIFIER ON
-- SET ANSI_NULLS ON
